...
-   Use the plot function on the county data frame with few more cases (plot with default values presented above): a) Once specifying the state to be "NC", the group being the top, the number looked at being 20

```{r}
plot(final_results[[1]],State = "NC", Order = "Top", n = 20)
```

b)  Once specifying the state to be "SC", the group being the bottom, the number looked at being 7

```{r}
plot(final_results[[1]], State = "SC", Order = "Bottom", n = 7)
```

c)  Once specifying the state to be "PA", the group being the top, the number looked at being 8
...
Plot function on the county data frame: a). Once specifying the state to be "CA", the group being the top, the number looked at being 15

```{r}
plot(f_results[[1]], State = "CA", Order = "Top", n = 15 )
```

b)  Once specifying the state to be "TX", the group being the top, the number looked at being 4

```{r}
plot(f_results[[1]], State = "TX", Order = "Top", n = 4 )
```

c)  Once without specifying anything (defaults used)

```{r}
plot(f_results[[1]])
```

d)  Once specifying the state to be "NY", the group being the top, the number looked at being 10
